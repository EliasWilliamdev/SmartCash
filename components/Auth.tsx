...
import { Mail, Lock, LogIn, UserPlus, Wallet, Loader2, AlertCircle, PlayCircle } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] p-6">
      <div className="w-full max-w-md bg-white rounded-[48px] shadow-2xl shadow-blue-900/10 border border-slate-100 overflow-hidden">
        <div className="bg-blue-600 p-12 text-center text-white">
          ...
          <p className="text-blue-100 mt-2 font-bold uppercase text-[10px] tracking-widest">Inteligência Financeira</p>
        </div>

        <div className="p-10 md:p-12">
          {error && (
            <div className="mb-8 p-4 bg-rose-50 border-2 border-rose-100 rounded-2xl flex items-center gap-3 text-rose-600 animate-in slide-in-from-top-2">
          ...
          <form onSubmit={handleAuth} className="space-y-6">
            ...
            >
              {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : (isSignUp ? 'CRIAR CONTA' : 'ACESSAR DASHBOARD')}
            </button>
          </form>
